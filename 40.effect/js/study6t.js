$(document).ready(function(){
    ...
    let fixedDivTop = fixedDiv.offset().top  // fixedEffect클래스 태그가 전체문서를 기준으로 했을 때 위에서부터 떨어져있는 거리를 픽셀로 계산
    ...
    $(window).scroll(function(){
        ...
        if(scrollDistance>=0 && scrollDistance<=7200){
            ...
            let percent2 = (scrollDistance/7200)*100
            console.log(count,percent,percent2)
            movingStation.css("position","fixed")
            movingStation.css("top","0")
            moveSlider(count)
            $(".textTrain").css("transform","translateY(-"+(percent2)+"%)")
    ...
    $(window).scroll(function(){
        ...
        let winheight = $(this).height()*0.7
        let fixedDivTop = $(".fixedEffect").offset().top
        let distance = winst+winheight-fixedDivTop
        if(distance>=0 && distance<winheight){
            let per40 = (distance)/winheight*40
            $(".station").css("width",(60+per40)+"%")
        }
    })
